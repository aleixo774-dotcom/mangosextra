...
const COLS: Status[] = ["recebido", "em_analise", "em_simulacao", "aprovado", "contrato", "pago"];
...
function Admin() {
  ...
  const aprovadas = referrals.filter((r) => r.status === "aprovado" || r.status === "contrato" || r.status === "pago").length;
  const pagas = referrals.filter((r) => r.status === "pago").length;
  const valorPipe = referrals.reduce((s, r) => s + r.commission, 0);
    ...
    <MobileShell>
      <header className="bg-forest px-5 pb-5 pt-12 text-forest-foreground">
        ...
        <div className="mt-5 grid grid-cols-4 gap-2">
          ...
          <Kpi label="Pipeline" value={brl(valorPipe)} small />
        </div>
      </header>
        ...
        <div className="mt-3 flex gap-1">
          {COLS.map((_, i) => (
            <button
              key={i}
              onClick={() => setCol(i)}
              className={`h-1.5 flex-1 rounded-full ${i === col ? "bg-mango" : "bg-border"}`}
            ...
            <li key={r.id} className="rounded-2xl bg-card p-4 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs font-semibold text-money">{brl(r.commission)}</p>
                </div>
                <Link
                  to="/indicacao/$id"
                  params={{ id: r.id }}
...
function Kpi({ label, value, small }: { label: string; value: string; small?: boolean }) {
  return (
    <div className="rounded-xl bg-white/10 p-2 backdrop-blur">
      ...
      <p className={`font-display font-bold ${small ? "text-xs" : "text-base"}`}>{value}</p>
    </div>
  );
}
