...
import { brl, POINTS_PER_APPROVED, PRODUCTS } from "@/lib/mango-data";
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success("Indicação enviada!", {
        description: `${r.client_name} • +${POINTS_PER_APPROVED} pts se aprovada`,
      });
    ...
    <MobileShell>
      ...
      <div className="px-5 pt-4">
        <div className="flex gap-1.5">
          {[1, 2, 3].map((i) => (
            <div key={i} className={`h-1.5 flex-1 rounded-full ${i <= step ? "bg-mango" : "bg-border"}`} />
          ))}
        </div>
      </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-mango/30 bg-mango/10 p-4">
              ...
              <p className="mt-1 font-display text-2xl font-bold text-forest">+{POINTS_PER_APPROVED} pts</p>
              <p className="text-xs text-muted-foreground">Creditados quando a indicação for aprovada.</p>
            </div>
          </div>
        )}
      ...
      <div className="sticky bottom-24 mt-6 px-5">
        {step < 3 ? (
          <button disabled={!canNext} onClick={() => setStep(step + 1)} className="w-full rounded-2xl bg-forest py-4 font-display font-semibold text-forest-foreground shadow-lg shadow-forest/30 disabled:opacity-50">
            Continuar
          </button>
        ) : (
          <button onClick={submit} disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-money py-4 font-display font-semibold text-money-foreground shadow-lg shadow-money/30 disabled:opacity-50">
            <Check className="h-5 w-5" /> {busy ? "Enviando…" : "Enviar Indicação"}
          </button>
        )}
      </div>
